...
export const StudentButton = ({ operation, children, student, onDone = () => {}, ...props }) => {
    ...
    const handleClick = async (params = {}) => {
        ...
        const transformedStudent = {
            ...student,
            ...params,
            // For create/update operations, we need to provide the required fields
            // The actual student name will be handled by the backend through the student relation
            userId: params.userId || student.userId || null,
            programId: params.programId || student.programId || null,
            stateId: params.stateId || student.stateId || null,
            semesterNumber: parseInt(params.semesterNumber) || student.semesterNumber || student.semester || 0
        };
        
        try {
            const freshStudent = await fetch(transformedStudent);
            if (freshStudent && freshStudent.data && freshStudent.data.result) {
                onDone(freshStudent.data.result); // Pass the result to the external callback
            } else {
                onDone(freshStudent); // Fallback if structure is different
            }
        } catch (error) {
            console.error("Student operation failed:", error);
            // Don't call onDone with error, let the error handler show the error
        }
    };
